...
object TestFTP  {
  def main(args: Array[String]): Unit = {
    ...
    val user = "root"
    val host = "10.15.191.238"
    val password = "123456"
    val folderFiles = "/root/"
    ...
    val stream = sftp.asInstanceOf[ChannelSftp].get(folderFiles + "gente.txt")
    ...
    //////////////////////////////////////////////////////
    /*Cuenta y conecta, y parsea*/
    while ({line = br.readLine; line != null}) {
      //   println(line)
      //   var uno = "ola,cosa,loca,tres"
      var lineDos = line.split('|')
      println(lineDos)
      var persona = DatosP(lineDos(0),lineDos(1).toInt,lineDos(2),lineDos(3),lineDos(4))
      println(persona)
      val gson = new Gson()
      ...
      /*
            ...
            val parse2 = parser.parse(json).getAsJsonObject
            ...
            var doc = MongoDBObject(
              "nombre" -> parse2.get("nombre"),
              "edad" -> parse2.get("edad"),
              "fechaNa" -> parse2.get("fechaNa"),
              "ciudad" -> parse2.get("ciudad"),
              "caracteristica" -> parse2.get("caracteristica")
            )
